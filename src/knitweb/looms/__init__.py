"""Domain looms — plugins that emit signed, attestable knowledge into the Web.

A *domain loom* turns domain facts (a chemical reaction, a financial statement, a
supply-chain event) into content-addressed, ECDSA-signed records that weave into
the fabric and validate at read time. Looms are **plugins, never core**: they
...
The contract every loom upholds:

  * emit only **physically/economically sound** records — each loom gates on a
