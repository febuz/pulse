"""#144 convergence-soundness: one signed record yields ONE Web shape on both
author and receiver — routing is decided by the ENVELOPE kind, never by the
author/attacker-controllable inner ``record['kind']`` field.
...
def test_node_record_converges_author_and_receiver() -> None:
    """A 'fabric-record' envelope is a NODE on both sides even when its inner
    record carries kind=='edge' with real src/dst (the #144 poison primitive)."""
    author = FabricNode()
    n1 = author.web.weave({"kind": "knowledge", "title": "A", "author": author.pub})
    n2 = author.web.weave({"kind": "knowledge", "title": "B", "author": author.pub})
    # A node-record whose inner kind=='edge' is attacker/author-controllable.
    poison = {"kind": "edge", "src": n1, "dst": n2, "rel": "supports", "weight": 1}
    envelope = author._signed_record_msg(poison)  # envelope kind == 'fabric-record'
    author.web.weave(poison)  # author ALWAYS stores it as a node
    ...
    # And it landed as a node, not an edge, on the receiver.
    assert sum(len(v) for v in receiver.web._out.values()) == 0
    assert len(receiver.web.nodes) == 3
